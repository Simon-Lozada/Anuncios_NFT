...
contract BlindAuction {
    ...
    function generateBlindedBidBytes32(uint value, bool fake) external view returns(bytes32){
    ...
    function auctionEnd() external payable onlyAfter(revealEnd){
        
        require(!ended);                                //Nos aseguramos que la subasta no alla terminado
    ...
    function withdraw() external{
        ...
        if (amount > 0){   
            ...
            payable(msg.sender).transfer(amount);   //Le enviamos el eter que aposto
        }
